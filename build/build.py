...
def check_numpy_version(python_bin_path):
  ...
  if numpy_version < (1, 20):
    print(f"ERROR: JAX requires NumPy 1.20 or newer, found {version}.")
    sys.exit(-1)
  return version
...
def get_bazel_version(bazel_path):
  ...
  return None if match is None else tuple(int(x) for x in match[1].split("."))
...
def main():
  ...
  print(f'Remote TPU enabled: {"yes" if args.enable_remote_tpu else "no"}')
  ...
  print(f'Plugin device enabled: {"yes" if args.enable_plugin_device else "no"}')
