...
class Var:
  ...
  def __lt__(self, other):
    return ((self.count, self.suffix) < (other.count, other.suffix)
            if isinstance(other, Var) else NotImplemented)
...
class Literal:
  ...
  def __repr__(self):
    return f'{self.val}' if hasattr(self, 'hash') else f'Literal(val={self.val})'
...
def _dim_handler_and_canonical(*dlist: DimSize) -> Tuple[DimensionHandler, Tuple[DimSize, ...]]:
  ...
  canonical = []
  for d in dlist:
    if handler := _get_special_dim_handler(d):
...
class NamedShape:
  ...
  def __str__(self):
    ...
    return (
        f"({', '.join(map(str, self.__positional))}{', ' if self.__named else ''}{', '.join(f'{k}={v}' for k, v in self.__named.items())})"
        if self.__named else str(self.__positional))
...
def _param_uses_outfeed(param):
  return bool(
      type(param) is Jaxpr and jaxpr_uses_outfeed(param)
      or type(param) is not Jaxpr and type(param) is ClosedJaxpr
      and jaxpr_uses_outfeed(param.jaxpr))
...
def do_subst_axis_names_jaxpr(jaxpr: Union[Jaxpr, ClosedJaxpr], subst: AxisSubst):
  ...
  return ClosedJaxpr(new_jaxpr, consts) if consts is not None else new_jaxpr
...
def check_type(
  ...
  if not isinstance(ty, DShapedArray):
...
def _check_call(ctx_factory, prim, in_atoms, params):
  ...
  def substitute(aval: AbstractValue):
    if isinstance(aval, DShapedArray):
      aval = aval.update(shape=tuple(env.get(d, d) for d in aval.shape))
    return aval
...
def _check_map(ctx_factory, prim, in_avals, params):
  ...
  call_jaxpr = params["call_jaxpr"]
  if ordered_effects_ := effects.ordered_effects.filter_in(call_jaxpr.effects):
    raise JaxprTypeError(
...
def pp_vars(vs: Sequence[Any], context: JaxprPpContext,
  ...
  if print_shapes:
    return pp.nest(
        ...
        pp.group(
            pp.join(
                pp.text(separator) + pp.group(pp.brk()),
                [(pp.text(pp_var(v, context)) + pp.type_annotation(
                    pp.text(f":{pp_aval(v.aval, context)}"))) for v in vs],
            )),
    )
  else:
  ...
  return (pp.group(
      pp.nest(
          ...
          pp.concat([
              ...
              pp.join(
                  pp.brk(),
                  [pp_kv_pair(k, v, context, settings) for k, v in kv_pairs],
              ),
          ]),
      ) + pp.brk("") + pp.text("]")) if kv_pairs else pp.nil())

def pp_eqn(eqn: JaxprEqn, context: JaxprPpContext, settings: JaxprPpSettings
           ) -> pp.Doc:
  rule = (pp_eqn_rules.get(eqn.primitive, _pp_eqn)
          if settings.custom_pp_eqn_rules else _pp_eqn)
...
def _compact_eqn_should_include(k: str, v: Any) -> bool:
  ...
  return not isinstance(v, tuple) or not any(
      isinstance(e, (Jaxpr, ClosedJaxpr)) for e in v)
...
def str_eqn_compact(primitive_name: str, params: Dict) -> str:
  ...
  return f"{primitive_name}[{kvs}]" if kvs != "" else primitive_name
